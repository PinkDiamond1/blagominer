...
TEST(RnD_BurstMath_CalcDeadline_QueLap, dccONburst641159WithGenSigFromDump) {
	...
	*/
	...
	auto currentSignature = HexString::arrayfrom<32>("BCEAA339BD7CD1B133749BF39F3EE6518CC94EA316FF7C49CD66F6D239A39EF5"); // 'corrected' gensig used by the miner
	uint64_t currentBaseTarget = 0x00008803; // 34819
	uint64_t account_nr = 7955621360090688183;

	{
		uint64_t nonce_nr = 0xB612771801CB0EE0 + 1;
		uint32_t scoop_nr = BurstMath::calculate_scoop(height, *currentSignature); // 0x0349
...
TEST(RnD_DcminerUnUPXed_GenerateGensig, dccONburst641159) {
	...
	*/
	...
	auto const height = 0x09C887; // 641159
	auto serverGenSig = HexString::arrayfrom<32>("ed377f7ea5f857c565baa9e44aa94f5c8162c2fdf430d6eccdac4b856c6b5b68"); // gensig from the pool
	uint64_t currentBaseTarget = 0x00008803; // 34819
	uint64_t account_nr = 7955621360090688183;

	auto currentSignature = diskcoin_generate_gensig_aes128(height, *serverGenSig);

	auto referencedata_ExpectedSignature = HexString::arrayfrom<32>("BCEAA339BD7CD1B133749BF39F3EE6518CC94EA316FF7C49CD66F6D239A39EF5"); // 'corrected' gensig used by the miner
	EXPECT_THAT(*currentSignature, ElementsAreArray(*referencedata_ExpectedSignature));
}
